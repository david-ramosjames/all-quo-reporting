...
/** Human-readable fragments for mandatory weekly sentiment escalation (matched rule ids). */
const ESCALATION_REASON_COPY = {
  bad_review_threat: 'public bad-review or reputation-threat language',
  ag_complaint: 'Attorney General or regulator-complaint language',
  bar_grievance: 'State Bar grievance or disciplinary language',
  performance_attack: 'explicit criticism of staff performance or case handling',
};

function weeklyBundleTextForEscalationScan(items) {
  const parts = [];
  for (const it of items) {
    if (it.recordType === 'sms') parts.push(String(it.body || ''));
    else {
      parts.push(String(it.summary || ''));
      if (String(it.transcript || '').trim()) parts.push(String(it.transcript));
    }
  }
  return parts.join('\n').toLowerCase();
}
...
/**
 * Deterministic escalation: if summaries/SMS match, force negative + high review risk
 * (overrides model). Used for explicit review threats, AG/bar language, and harsh performance criticism.
 * @returns {string[]} unique matched rule ids
 */
function matchWeeklySentimentEscalations(bundleLower) {
  const matched = new Set();
  const patterns = [
    {
      id: 'bad_review_threat',
      re: /\b(threat|threaten|threatening|threatened)\b.{0,80}\b(bad\s+)?(review|rating|yelp|google|online|social)\b/i,
    },
    {
      id: 'bad_review_threat',
      re: /\b(bad|negative|honest|scathing|terrible|1[\s-]*star|one[\s-]*star)\s+review\b/i,
    },
    {
      id: 'bad_review_threat',
      re: /\b(leave|leaving|post|posting|write|writing|put)\b.{0,45}\b(bad|negative|1[\s-]*star|one[\s-]*star)?\s*(review|rating)\b/i,
    },
    {
      id: 'bad_review_threat',
      re: /\b(google|yelp|facebook|bbb|better business bureau)\b.{0,55}\b(review|rating|stars)\b/i,
    },
    {
      id: 'ag_complaint',
      re: /\b(complaint|file|filing|report)\b.{0,110}\b(attorney\s+general|a\.?\s*g\.?\s*office|office\s+of\s+the\s+attorney\s+general)\b/i,
    },
    {
      id: 'ag_complaint',
      re: /\b(attorney\s+general|attorney\s+generals)\b.{0,120}\b(complaint|file|filing|report|letter|notify)\b/i,
    },
    {
      id: 'bar_grievance',
      re: /\b(texas\s+)?state\s+bar\b.{0,100}\b(complaint|grievance|disciplinary|discipline|ethics)\b/i,
    },
    {
      id: 'bar_grievance',
      re: /\bbar\s+(complaint|grievance)\b/i,
    },
    {
      id: 'bar_grievance',
      re: /\b(grievance|complaint)\b.{0,80}\b(texas\s+)?state\s+bar\b/i,
    },
    {
      id: 'bar_grievance',
      re: /\b(file|filing)\s+(a\s+)?(formal\s+)?(complaint|grievance)\b.{0,120}\b(state\s+bar|bar\s+association)\b/i,
    },
    {
      id: 'performance_attack',
      re: /\b(didn'?t|did\s+not)\s+like\b.{0,90}\bperformance\b/i,
    },
    {
      id: 'performance_attack',
      re: /\b(your|our|this)\s+(firm|office|lawyer|attorney|paralegal|staff|representation)\b.{0,100}\b(negligent|negligence|malpractice|incompetent|unprofessional)\b/i,
    },
    {
      id: 'performance_attack',
      re: /\b(negligent|negligence|malpractice|incompetent|unprofessional)\b.{0,80}\b(your|our|this)\s+(firm|office|lawyer|attorney|paralegal|staff)\b/i,
    },
    {
      id: 'performance_attack',
      re: /\b(not\s+happy|unhappy|dissatisfied)\b.{0,120}\b(how\s+.{3,48}\s+handled|your\s+office|this\s+firm|your\s+firm)\b/i,
    },
  ];
  for (const { id, re } of patterns) {
    if (re.test(bundleLower)) matched.add(id);
  }
  return [...matched];
}

function applyMandatorySentimentEscalation(analysis, escalationIds) {
  if (!analysis || !escalationIds.length) return analysis;
  analysis.sentiment = 'negative';
  analysis.bad_review_risk = 'high';
  analysis.positive_review_candidate = 'none';
  analysis.positive_review_note = '';
  analysis.reason_tags = normalizeReasonTags([
    ...analysis.reason_tags,
    'regulatory_or_review_threat',
    'frustration',
  ]);
  const bits = escalationIds.map((id) => ESCALATION_REASON_COPY[id] || id);
  const auto =
    'Automated escalation: touchpoint text matched ' +
    [...new Set(bits)].join('; ') +
    '.';
  analysis.bad_review_risk_note = analysis.bad_review_risk_note
    ? `${auto} ${analysis.bad_review_risk_note}`
    : auto;
  const extra =
    ' Firm rule: explicit review/regulatory threats or severe performance criticism require negative sentiment and high public-risk rating.';
  const rs = (analysis.reason_summary || '').trim();
  if (!/\bfirm rule:\b/i.test(rs)) {
    analysis.reason_summary = `${rs.replace(/\s*\.\s*$/, '')}.${extra}`.trim();
  }
  const cs = (analysis.client_state || '').trim();
  if (cs && !/\bescalation risk\b/i.test(cs)) {
    analysis.client_state = `${cs.replace(/\s*\.\s*$/, '')} Escalation risk is high.`.trim();
  } else if (!cs) {
    analysis.client_state = 'Escalation risk is high.';
  }
  return analysis;
}
...
async function analyzeWeeklyClientBundleWithLlm(clientKey, items, rangeLabel, attempt = 1) {
  ...
  if (parsed) {
    const bundleLower = weeklyBundleTextForEscalationScan(items);
    const escalationIds = matchWeeklySentimentEscalations(bundleLower);
    if (escalationIds.length) {
      applyMandatorySentimentEscalation(parsed, escalationIds);
      console.log(
        `  Mandatory sentiment escalation (${escalationIds.join(', ')}) — ${clientKey.slice(0, 44)}`
      );
    }
    return parsed;
  }
